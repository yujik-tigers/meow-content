...
Create an image of a cat based on this movie quote and user feedback.
...
{prompt}
    """
        ...
        text_rendered_image = image_text_renderer.add_text(
            new_image, content.content, f"{content.author}, {content.title}"
        )

        timestamp = datetime.now().strftime("%y%m%d%H%M%S")
        image_url = await self._image_storage.upload_image(
            text_rendered_image,
            f"literal_quote/{date.today()}/{content.id}/edited/{timestamp}.png",
        )
